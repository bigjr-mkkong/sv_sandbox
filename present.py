...
def print_sbox():
    for i in range(0, 256):
        ...
        pad2 = ""
        if sbox[i] >=0 and sbox[i] <= 9:
            pad2 = "  "
        elif sbox[i] >=10 and sbox[i] <= 99:
            pad2 = " "
        else:
            pad2 = ""

        strr = "8'd"+str(i)+pad1 + ": data_o=" + "8'd"+str(sbox[i]) + pad2+ ";"
        print(strr)
        strr = ""
...
# 32bit input P and K
# step1: xor
# step2: split into four bytes, then sbox
# step3: pbox
...
def pbox_app(m: int, pbox: list[int]) -> int:
    output = 0
    for i in range(32):
        bit = (m >> pbox[i]) & 1
        output |= (bit << i)
